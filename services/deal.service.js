...
export const reqCreateActivity = async (req, res) => {
    try {
        const {shopId, productIds, title, startTimeUnix, endTimeUnix} = req.body;
...
export const reqUpdateActivityProduct = async (req, res) => {
    try {
        const {shopId, productIds, activityId, dealDiscount, dealQtyLimit, dealQtyPerUser} = req.body;
        ...
        for (let i = 0; i < productIds.length; i++) {   
            ...
            if (productResponse.data.data) {
                ...
                for (let j = 0; j < productResponse.data.data.skus.length; j++) {
                    productSkus.push({
                        ...
                        "quantity_limit": parseInt(dealQtyLimit),
                        "quantity_per_user": parseInt(dealQtyPerUser)
                    });
                }
            }
            ...
            products.push({
                "id": productIds[i],
                'discount': dealDiscount,
                'quantity_limit': dealQtyLimit,
                'quantity_per_user': dealQtyPerUser,    
                "skus": productSkus,                
            });
        }
...
export const reqRemoveActivityProduct = async (req, res) => {
    try {
        ...
        if (response.data.data) {
            ...
            return res.status(500).json({ message: "Failed to remove activity product" });
        }
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: error.message });
...
export const reqSearchCoupons = async (req, res) => {
    try {
        const {shopId, pageSize, pageToken, status, titleKeyword, displayType} = req.body;
